...
public class BrokerLinkTest {
    public static void main(String[] args) {
        ...
        String accountJson = "[\n" +
                ...
                "    }\n" +
                "]";
        //拿到 broker link list 422
        List<BrokerLink> brokerLinkList = JSONArray.parseArray(brokerlinkJson,BrokerLink.class);
        Map<String,BrokerLink> brokerMap = new HashMap<String, BrokerLink>();
        for(BrokerLink brokerLink:brokerLinkList){
            brokerMap.put(brokerLink.getUserId(),brokerLink);
        }
        Map<String,BrokerLink> needDeleteBrokerMap = new HashMap<String, BrokerLink>();

        // 拿到可能有问题的 account list 253
        List<Account> accountsList = JSONArray.parseArray(accountJson,Account.class);
        //把 account list 映射为 key = account email， value = account_id
        Map<String,String> accountMap = new HashMap<String, String>();
        for(Account account:accountsList){
            if(account.getEmail().contains("deleted_")){
                //如果 account email 包含 deleted_ 那么就删除对应的 broker link
                BrokerLink needDelteBroker = brokerMap.get(account.getAccount_id());
                String key = needDelteBroker.getProviderId()+needDelteBroker.getBrokerUserId()+needDelteBroker.getUserId();
                needDeleteBrokerMap.put(key,needDelteBroker);
            }else {
                accountMap.put(account.getEmail(),account.getAccount_id());
            }
        }
        ...
        for(BrokerLink info: brokerLinkList){
            // 遍历 broker link list, 如果 该 broker link 的 broker email 跟 account email 一致，且绑定的 account id 一致，那么就不删除，否则就生成删除该 broker link 的 SQL
            if(accountMap.get(info.getEmail())!=null){
                if(info.getUserId().equals(accountMap.get(info.getEmail()))){
                    undeleteCount++;
                    continue;
                }
            } else if(info.getProviderId().equals("google")){
                googleList.add(info.getEmail());
                for(Account account : accountsList){
                    if(account.getAccount_id().equals(info.getUserId())){
                        System.out.println("broker email:"+info.getEmail()+",account email:"+account.getEmail()+",account_Id"+account.getAccount_id());
                        break;
                    }
                }
            }
            String key = info.getProviderId()+info.getBrokerUserId()+info.getUserId();
            needDeleteBrokerMap.put(key,info);
           deleteCount++;

        }
        Iterator<Map.Entry<String, BrokerLink>> iterator= needDeleteBrokerMap.entrySet().iterator();
        while (iterator.hasNext()){
            BrokerLink brokerLink = iterator.next().getValue();
        ...
        //1.找出 delete_开头的 account email，并且把对应的 broker link 全删掉
        //2.对于剩余的 account_id,
        System.out.println("total:"+brokerLinkList.size()+", undeleteCount:"+undeleteCount+"deleteCount:"+deleteCount);
